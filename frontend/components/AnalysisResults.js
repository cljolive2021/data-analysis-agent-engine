    ...
    <div className="space-y-6">
      <Section title="📊 Resumo dos Dados" section="summary" expanded={expandedSections.summary} onToggle={toggleSection}>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard label="Linhas" value={summary.total_rows} icon="📈" />
          <StatCard label="Colunas" value={summary.total_columns} icon="📋" />
          <StatCard label="Numéricas" value={summary.numeric_columns} icon="🔢" />
          <StatCard label="Categóricas" value={summary.categorical_columns} icon="🏷️" />
        </div>
      </Section>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {targetAnalysis.type === 'numeric' && (
            <>
              ...
              <StatCard label="Desvio" value={targetAnalysis.std?.toFixed(2)} icon="📉" />
              <StatCard label="Mínimo" value={targetAnalysis.min?.toFixed(0)} icon="⬇️" />
              <StatCard label="Máximo" value={targetAnalysis.max?.toFixed(0)} icon="⬆️" />
            </>
          )}
        </div>
      </Section>
              ...
              <ul className="space-y-2">
                {topCorrelations.map((corr, idx) => (
                  <li key={idx} className="flex justify-between items-center text-sm text-slate-300">
                    ...
                    <span className="font-mono font-bold px-3 py-1 rounded bg-slate-500/20 text-slate-300">
